...
describe SectionsContext do
  ...
  describe '#form' do
    it 'returns form' do
      expect(ExerciseSettingsForm).to receive(:new)
        .with(included_verbs: %w(1 2 3), excluded_verbs: nil).and_return(form)
  ...
  describe '#presenter' do
    it 'returns presenter' do
      expect(ExerciseFormPresenter).to receive(:new)
        .with(params).and_return(presenter)
